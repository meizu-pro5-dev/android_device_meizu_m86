...
Return<void> Sensors::poll(int32_t maxCount, poll_cb _hidl_cb) {
    ...
    { // scope of reentry lock
        ...
        if (maxCount <= 0) {
            ...
            do {
                const auto before = m86::sampleClocks();
                err = mSensorDevice->poll(
                        reinterpret_cast<sensors_poll_device_t *>(mSensorDevice),
                        data.get(), bufferSize);
                const auto after = m86::sampleClocks();
                if (err > bufferSize) {
                    err = android::BAD_VALUE;
                    break;
                }
                if (err > 0) {
                    ...
                    for (int i = 0; i < err; ++i) {
                        auto &event = data[i];
                        if (m86::isLegacyInputSensor(event.sensor, event.type)) {
                            int64_t normalized;
                            if (!m86::normalizeTimestamp(event.timestamp, before, after,
                                                         &normalized)) {
                                LOG(WARNING) << "m86 sensors: dropping ambiguous ALS/PS timestamp";
                                continue;
                            }
